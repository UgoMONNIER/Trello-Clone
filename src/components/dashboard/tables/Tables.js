...
export const Tables = () => {
  ...
  const getData = async () => {
    ...
    try {
      let response = await fetch(`http://localhost:5000/tables/${params.uid}/${params.id_space}/getAllTables`, initConfig)
      let result = await response.json()
      result.map((table, index)=>(
          

        table.img ="https://picsum.photos/20"+(index)+"/300"
      ))
      ...
      <Grid container spacing={2}>
        {
          tablesData.map((table) => (
            ...
            <Grid item lg={3} key={table.id}>
              <TableCard img={table.img} id={table.id} imgUrl="#" title={table.title} description={table.description} />
            </Grid>
          ))
        }
        <Grid item lg={3}>
          <AddTables reloadSpaceContent={reloadSpaceContent} />
        </Grid>
      </Grid>
    </div>
  );
