...
export const SpaceContent = () => {
  ...
  const getData = async () => {
    ...
    try {
      let response = await fetch(`http://localhost:5000/spaceswork/${params.uid}/getAllSpaces`, initConfig)
      let result = await response.json()
      // result.map((espace, index)=>(


      //   espace.img ="https://picsum.photos/20"+(index)+"/300"
      // ))
      console.log(result)
          ...
          spacesData.map((space) => {
            ...
            return (
              <Grid item lg={3} key={space.id}>
                <SpaceCard img={space.img} id={space.id} imgUrl="#" title={space.title} description={space.description} />
              </Grid>
            )
          }
          )
