...
import { TabsRoutesDashboardColumns,TabsRoutesDashboardTables, TabsRoutesDashboard } from "./tabsConfigDashboard";
...
const DashboardColumns = ({window}) => {
  ...
  const drawer = (
    ...
    <div>
      ...
      <Link to={`/admin/${params.uid}/sp_w/${params.id_space}`}
          style={{textDecoration: "none"}}
          >
       <Tab
            sx={{ width: { sm: drawerWidth },mt: 10,color: "white"}}
            icon={TabsRoutesDashboardTables[0].icon}
            label={TabsRoutesDashboardTables[0].label}
            id={`vertical-tab-tableaux`}
            aria-controls={`vertical-tabpanel-tableaux`}
          />
      </Link>
    ...
    <Box sx={{ display: "flex" }}>
      ...
      >
        ...
        >
          ...
          {/* contenu des onglets */}
          <div>
          {TabsRoutesDashboardColumns.map((tab, index) => (
